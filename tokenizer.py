...
class VortanTokenizer:
    SEGMENTATION_RULES = [
        ...
        (5, u'[' + Punct.all() + ']\s*[' + Punct(1).regex() + \
            ']{1}\s*[Ա-ֆևևA-zА-яЁё0-9]+', 1),  # . <<
        (6, u'\.{1}\n', 1),
        # : — Ա (vortan)
        (99, u'(' + Punct(':').regex() + ')\s*—[ ]?[Ա-ՖևA-ZА-ЯЁ]+', 1),
    ]

    WORD_TOK_RULES = [
        # ՀՀԿ-ական ( լավ չի, բայց ուրիշ օրինակ մտքիս չեկավ )
        (11, u'[Ա-Ֆևа-яА-ЯЁёA-z]+-[ա-ֆև]+'),
        (13, u'[ա-ֆԱ-Ֆևև]+'),  # simple word
        # հեյ~(հե~յ)
        (20, u'[ա-ֆԱ-Ֆևև]+[' + Punct.all(linear=True) + ']{1,3}'),
    ]
    ...
    PUNC_TOK_RULES = [
        ...
        # all linear punctuations
        (18, u'([' + Punct.all(linear=True) + ']{1})'),
        (16, u'\.{3,4}'),  # .... , ...
    ]
    ...
    SPECIAL_RULES = {
        'segment': [
            ...
            ('__all__', False, u'[' + Punct(1).regex() + \
             ']\s*[Ա-ՖևA-ZА-ЯЁ]{1}[A-zА-яЁёԱ-ֆևև\s։]+[^' + Punct(2).regex() + ']$'),
    ...
    PURIFICATION_RULES = [
        ('<<', '«'),
        ('>>', '»'),
        ('(?P<w_beg>[ա-ֆԱ-Ֆևև]+)(?P<symbol>[' + Punct.all(linear=True) +
         ']){1}(?P<w_end>[ա-ֆԱ-Ֆևև]*)', '\g<w_beg>\g<w_end>\g<symbol>'),  # LINEAR_PUNCTUATION
    ...
    def __init__(self, text, tok_punc=False, tok_foreign=False, tok_numb=False, tok_misc=False):
        ...
        self.segments = []
        self.tok_rules = VortanTokenizer.WORD_TOK_RULES
        if tok_punc:
            self.tok_rules += VortanTokenizer.PUNC_TOK_RULES
        if tok_foreign:
            self.tok_rules += VortanTokenizer.FOREIGN_TOK_RULES
        if tok_numb:
            self.tok_rules += VortanTokenizer.NUMBER_TOK_RULES
        if tok_misc:
            self.tok_rules += VortanTokenizer.MISC_TOK_RULES
    ...
    def print_(self):
        ...
        for s in self.segments:
            output += '{num}. {string}\n{line}\n'.format(
                num=s['id'], string=s['segment'], line='-' * 50)
            for t in s['tokens']:
                output += '{token}\n'.format(token=t)
                ...
                if 'exist' in dict_word and dict_word['exist'] == True:
                    ...
                    if token:
                        ...
                        if multi:
                            ...
                            s['tokens'].append(
                                ('{s}-{e}'.format(s=start_p, e=end_p), clean_token))
                            for t in multi:
